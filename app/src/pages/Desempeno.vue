...
export default {
  ...
  setup () {
    ...
    const loadUserData = () => {
        $store.dispatch('api/GetSingleData',  {coll: user.value.group+'s', user_id: user.value.id}).then(res => {
    ...
    const loadItems = () => {
      ...
      $store.dispatch('api/GetAllDataFilteredV2', ['asesores',req ]).then(res => {
        ...
        //Load student's info
        var reqEstudiante = { user_id: asesor.value.asignacion.estudiante.user_id }
