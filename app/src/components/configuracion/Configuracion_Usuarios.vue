...
div
    ...
    q-table.shadow-0(:loading="loading" no-data-label="Sin datos" :rows="datausers" :columns="columns" dense)
        template(v-slot:top)
            .flex.justify-end.full-width: q-btn(label="Nuevo usuario" color="secondary" rounded unelevated @click="edituserDialog(cleanuser, true)"  no-caps)
        template(v-slot:loading)
            q-inner-loading(showing color="primary")
        
        template(v-slot:body="props")
            q-tr(:props="props" @click="edituserDialog(props.row, false)").cursor-pointer
    ...
    q-dialog(v-model="dialog")
        q-card.bg-dark.shadow-20
            q-card-section.text-center: .text-h6.text-white Editar usuario
            q-card-section
                q-form
                    ...
                    q-separator(spaced)
                    .text-center.q-mt-sm
                        q-btn-toggle(rounded unelevated toggle-color="info" text-color="white" v-model="edituser.group" :options="[ {label: 'Docente', value: 'docente'}, {label: 'Estudiante', value: 'estudiante'}, {label: 'Coordinador', value: 'coordinador'}, {label: 'Admin', value: 'admin'}]")
                    q-separator(spaced)
                    .text-center.q-mt-sm
                        q-toggle(v-model="edituser.active" label="Activo" color="info").q-mt-sm.text-white
                    .text-center.q-mt-lg
                        q-btn(label="Guardar" color="secondary" rounded unelevated @click="edituserFN")
...
export default {
    setup () {
        ...
        const columns = ref([
            ...
            { name: 'group', label: 'Tipo', field: 'group', align:'left', sortable: true },
