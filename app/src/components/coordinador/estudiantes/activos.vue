...
export default {
    setup(){
        ...
        const columns = [
            { name: 'user', label: 'Nº de Cuenta', field: row=> row.user.user, align:'left', sortable: true},
            { name: 'name', label: 'Nombre', field: row=>row.user.name, align:'left' },
        ...
        const gotoStudent = (item) => {
            const path = '/estudiante/'+item.user_id
            $router.push(path)
        }
